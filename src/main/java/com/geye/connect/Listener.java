package com.geye.connect;

import com.geye.deallogs.TimmerToDo;
import com.geye.model.XmlModel;

import redis.clients.jedis.Jedis;
import redis.clients.jedis.JedisPubSub;

public class Listener extends JedisPubSub {
	
	private XmlModel xmlModelinfo;
	
	private Jedis subJedis;
	
	TimmerToDo timmerToDo = new TimmerToDo();
	
	public Listener(XmlModel xmlModelinfo,Jedis subJedis){	
		this.subJedis = subJedis;
		this.xmlModelinfo = xmlModelinfo;
	}
	
	// 取得订阅的消息后的处理
	public void onMessage(String channel, String message) {
		
		timmerToDo.setmessage(message, xmlModelinfo,subJedis);
	}
	
	// 初始化订阅时候的处理
	public void onSubscribe(String channel, int subscribedChannels) {
		// System.out.println(channel + "=" + subscribedChannels);
	}

	// 取消订阅时候的处理
	public void onUnsubscribe(String channel, int subscribedChannels) {
		// System.out.println(channel + "=" + subscribedChannels);
	}

	// 初始化按表达式的方式订阅时候的处理
	public void onPSubscribe(String pattern, int subscribedChannels) {
		// System.out.println(pattern + "=" + subscribedChannels);
	}

	// 取消按表达式的方式订阅时候的处理
	public void onPUnsubscribe(String pattern, int subscribedChannels) {
		// System.out.println(pattern + "=" + subscribedChannels);
	}

	// 取得按表达式的方式订阅的消息后的处理
	public void onPMessage(String pattern, String channel, String message) {
		System.out.println(pattern + "=" + channel + "=" + message);
	}


	public XmlModel getXmlModelinfo() {
		return xmlModelinfo;
	}


	public void setXmlModelinfo(XmlModel xmlModelinfo) {
		this.xmlModelinfo = xmlModelinfo;
	}

}
